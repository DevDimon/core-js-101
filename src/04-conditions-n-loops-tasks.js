...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  if (isStartIncluded) result += `[${a1}, ${b1}`;
  else result += `(${a1}, ${b1}`;
...
function isCreditCardNumber(ccn) {
  const ccNumber = ccn.toString();

  const digits = ccNumber.split('').map(Number);
...
function getDigitalRoot(num) {
  ...
  let num1 = num;
  while (num1 > 9) {
    summa = num1.toString().split('').reduce((sum, val) => sum + Number(val), 0);
    if (summa <= 9) return summa;
    num1 = summa;
  }
  return num1;
}
...
function getCommonDirectoryPath(pathes) {
  let shortestPath = pathes[0];
  for (let i = 1; i < pathes.length; i += 1) {
    if (pathes[i].length < shortestPath.length) {
      shortestPath = pathes[i];
    }
  }
  ...
  if (commonPrefix.length > 0 && commonPrefix[commonPrefix.length - 1] !== '/') {
